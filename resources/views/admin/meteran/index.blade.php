...
<!-- Start block -->
      ...
      <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden ">
          <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4 ">
              ...
              <div class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                  <button type="button" id="*" data-modal-target="addkamar" data-modal-toggle="addkamar" class="flex items-center justify-center text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="sticky top-0 z-10 text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-4">Status kamar</th>
    ...
    <tr class="border-b dark:border-gray-700">
        ...
        <td class="px-4 py-3">{{ $room->jenis_kamar }}</td>
        <td class="px-4 py-3">{{ $room->harga_kamar }}</td>
        <td class="px-4 py-3">{{ $room->status_kamar }}</td>
        <td class="px-4 py-3 flex items-center justify-end">
            <!-- Action Button -->
            <button id="action-dropdown-kamar{{ $index }}" 
                data-dropdown-toggle="dropdown-kamar{{ $index }}" 
            ...
            <!-- Dropdown Menu -->
            <div id="dropdown-kamar{{ $index }}" 
                class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                    <ul class="py-1 text-sm" aria-labelledby="action-dropdown-kamar{{ $index }}">
                                        <li>
                                            <button 
                                                type="button"  
                                                data-modal-target="updatekamar-{{ $room->id }}"
                                                data-modal-toggle="updatekamar-{{ $room->id }}"
...
{{--- model update/edit kamar---------------------------------------------------------------------------------------------------}}
    ...
    <div id="updatekamar-{{ $kamar->id }}" tabindex="-1" aria-hidden="true" 
        ...
        <div class="relative w-full max-w-lg max-h-full bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <!-- Modal Form -->
            <form method="POST" action="{{ route('kamar.update', $kamar->id) }}" class="p-6">
                @csrf
                @method('PUT')
            
                <!-- Room Name -->
                <div class="mb-4">
                    <label for="nomer_kamar-{{ $kamar->id }}" class="block text-sm font-medium text-gray-700 dark:text-gray-200">
                        Room Name
                    </label>
                    <input type="text" id="nomer_kamar-{{ $kamar->id }}" name="nomer_kamar" value="{{ $kamar->nomer_kamar }}" 
                        class="block w-full mt-1 border-gray-300 rounded-md shadow-sm sm:text-sm">
                </div>
            
                <!-- Room Type -->
                <div class="mb-4">
                    <label for="jenis_kamar-{{ $kamar->id }}" class="block text-sm font-medium text-gray-700 dark:text-gray-200">
                        Room Type
                    </label>
                    <input type="text" id="jenis_kamar-{{ $kamar->id }}" name="jenis_kamar" value="{{ $kamar->jenis_kamar }}" 
                        class="block w-full mt-1 border-gray-300 rounded-md shadow-sm sm:text-sm">
                </div>
            
                <!-- Room Status -->
                <div class="mb-4">
                    <label for="status_kamar-{{ $kamar->id }}" class="block text-sm font-medium text-gray-700 dark:text-gray-200">
                        Room Status
                    </label>
                    <select id="status_kamar-{{ $kamar->id }}" name="status_kamar" 
                        class="block w-full mt-1 border-gray-300 rounded-md shadow-sm sm:text-sm">
                        <option value="available" {{ $kamar->status_kamar == 'available' ? 'selected' : '' }}>Available</option>
                        <option value="occupied" {{ $kamar->status_kamar == 'occupied' ? 'selected' : '' }}>Occupied</option>
                    </select>
                </div>
            
                <!-- Room Price -->
                <div class="mb-4">
                    <label for="harga_kamar-{{ $kamar->id }}" class="block text-sm font-medium text-gray-700 dark:text-gray-200">
                        Room Price
                    </label>
                    <input type="number" id="harga_kamar-{{ $kamar->id }}" name="harga_kamar" value="{{ $kamar->harga_kamar }}" 
                        class="block w-full mt-1 border-gray-300 rounded-md shadow-sm sm:text-sm">
                </div>
  ...
  <div id="deleteModalkamar-{{ $kamar->id }}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full bg-black bg-opacity-50">
      <div class="relative p-4 w-full max-w-md max-h-full">
          ...
          <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
              <button type="button" class="text-gray-400 absolute top-2.5 right-2.5 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="deleteModalkamar-{{$kamar->id }}">
              ...
              <p class="mb-4 text-gray-500 dark:text-gray-300">Are you sure you want to delete this kamar?</p>
              <p class="mb-4 text-gray-500 dark:text-gray-300">"{{$kamar->nomer_kamar}}"</p>
              <form action="{{ route('kamar.destroy', $kamar->id) }}" method="POST">
                  @csrf
                  @method('DELETE')
                  <div class="flex justify-center items-center space-x-4">
                      <button data-modal-hide="deleteModalkamar-{{$kamar->id }}" type="button" class="py-2 px-3 text-sm font-medium text-gray-500 bg-white rounded-lg border border-gray-200 hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-primary-300 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">
                          No, cancel
                      </button>

<x-app-layout>
                ...
                <tbody>
                    @foreach ($meters as $index => $meter)
                      <tr class="border-b dark:border-gray-700">
                          ...
                          <td class="px-4 py-3">{{ $meter->meters }}</td>
                              ...
                              <div id="dropdown-meter{{ $index }}" 
                                ...
                                <ul class="py-1 text-sm" aria-labelledby="action-dropdown-meter{{ $index }}">
                                    <li>
                                        <button 
                                            type="button"  
                                            data-modal-target="updatemeter-{{ $meter->id_meters }}"
                                            data-modal-toggle="updatemeter-{{ $meter->id_meters }}"
                                          ...
                                          <li>
                                              <button 
                                                  type="button" 
                                                  data-modal-target="deleteModalMeter-{{ $meter->id_meters }}" 
                                                  data-modal-toggle="deleteModalMeter-{{ $meter->id_meters }}" 
  ...
  {{-- catatan, lebih baik ubah $kamar ke variable beda setiap edit,delete dan view model kah? --}}
      ...
      <div id="updatemeter-{{ $meter_update->id_meters }}" tabindex="-1" aria-hidden="true" 
          ...
          <div class="relative w-full max-w-lg max-h-full bg-white rounded-lg shadow dark:bg-gray-700">
              ...
              <div class="flex items-start justify-between p-4 border-b rounded-t dark:border-gray-600">
                  ...
                  <button type="button" 
                      ...
                      data-modal-hide="updatemeter-{{ $meter_update->id_meters }}">
              ...
              <!-- Modal Form -->
              <form method="POST" action="{{ route('meter.update', $meter_update->id_meters) }}" class="p-6">
                  @csrf
                  @method('PUT')
              
                  <!-- Meter Reading -->
                <div class="mb-4">
                    <label for="meters-{{ $meter_update->id_meters }}" class="block text-sm font-medium text-gray-700 dark:text-gray-200">
                        Meter Reading
                    </label>
                    <input type="number" id="meters-{{ $meter_update->id_meters }}" name="meters" value="{{ $meter_update->meters }}" 
                        class="block w-full mt-1 border-gray-300 rounded-md shadow-sm sm:text-sm">
                </div>
            
                <!-- Month -->
                <div class="mb-4">
                    <label for="month-{{ $meter_update->id_meters }}" class="block text-sm font-medium text-gray-700 dark:text-gray-200">
                        Month
                    </label>
                    <input type="date" id="month-{{ $meter_update->id_meters }}" name="month" value="{{$meter_update->month}}"
                        class="block w-full mt-1 border-gray-300 rounded-md shadow-sm sm:text-sm">
                </div>
  ...
  {{-- model delete kamar --------------------------------------------------------------------------}}
    ...
    <div id="deleteModalMeter-{{ $meter->id_meters }}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full bg-black bg-opacity-50">
        <div class="relative p-4 w-full max-w-md max-h-full">
            <!-- Modal content -->
            <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                <button type="button" class="text-gray-400 absolute top-2.5 right-2.5 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="deleteModalMeter-{{$meter->id_meters }}">
                ...
                <p class="mb-4 text-gray-500 dark:text-gray-300">Meter ID: "{{$meter->id_meters}}"</p>
                <form action="{{ route('meter.destroy', $meter->id_meters) }}" method="POST">
                    @csrf
                    @method('DELETE')
                    <div class="flex justify-center items-center space-x-4">
                        <button data-modal-hide="deleteModalMeter-{{$meter->id_meters }}" type="button" class="py-2 px-3 text-sm font-medium text-gray-500 bg-white rounded-lg border border-gray-200 hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-primary-300 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">
                            No, cancel
                        </button>

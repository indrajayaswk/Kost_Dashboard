{{--- model update/edit kamar---------------------------------------------------------------------------------------------------}}
  ...
  <div id="updateroom-{{ $kamar->id }}" tabindex="-1" aria-hidden="true" 
      ...
      <div class="relative w-full max-w-lg max-h-full bg-white rounded-lg shadow dark:bg-gray-700">
          ...
          <!-- Modal Form -->
          <form method="POST" action="{{ route('room.update', $kamar->id) }}" enctype="multipart/form-data" class="p-6">
            @csrf
            @method('PUT')
        
            <!-- Room Name -->
            <div class="mb-4">
                <label for="room_number" class="block text-sm font-medium text-gray-700">Room Name</label>
                <input type="text" id="room_number" name="room_number" value="{{ $kamar->room_number }}" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm sm:text-sm">
            </div>
        
            <!-- Room Type -->
            <div class="mb-4">
                <label for="room_type" class="block text-sm font-medium text-gray-700">Room Type</label>
                <input type="text" id="room_type" name="room_type" value="{{ $kamar->room_type }}" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm sm:text-sm">
            </div>
        
            <!-- Room Status -->
            <div class="mb-4">
                <label for="room_status" class="block text-sm font-medium text-gray-700">Room Status</label>
                <select id="room_status" name="room_status" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm sm:text-sm">
            ...
            <div class="mb-4">
                ...
                <input type="number" id="room_price" name="room_price" value="{{ $kamar->room_price }}" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm sm:text-sm">
            </div>
        
            <!-- Current Images -->
            @if ($kamar->room_images)
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700">Current Images</label>
                    <div class="flex space-x-4">
                        @foreach ($kamar->room_images as $image)
            ...
            <!-- Upload New Images -->
            <div class="mb-4">
                <label for="room_images" class="block text-sm font-medium text-gray-700">New Images</label>
                <input type="file" id="room_images" name="room_images[]" multiple class="block w-full mt-1 border-gray-300 rounded-md shadow-sm sm:text-sm">
            </div>
        
            <!-- Save Button -->
            <div class="flex items-center justify-end space-x-4">
                <button type="button" class="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-200 rounded-lg hover:bg-gray-300" data-modal-hide="updateroom-{{ $kamar->id }}">Cancel</button>

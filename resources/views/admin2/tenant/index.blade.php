<x-app-layout>
    <div class="p-6">
        ...
        <div class="flex flex-col md:flex-row items-center justify-between mb-6">
            <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-200">Tenant Management</h2>
            <div class="flex items-center space-x-4 mt-4 md:mt-0">
                ...
                <form action="{{ route('tenant.index') }}" method="GET" class="flex items-center">
                    ...
                    </div><button id="filterDropdownButton" data-dropdown-toggle="filterDropdown" class="w-full md:w-auto flex items-center justify-center py-2 px-4 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-primary-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700" type="button">
                ...
                <tbody>
                    @foreach ($tenants as $index => $tenant)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            <td class="px-6 py-4">{{ $tenants->firstItem() + $index }}</td>
                            <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                {{ $tenant->name }}
                            </th>
                            <td class="px-6 py-4">{{ $tenant->phone }}</td>
                            <td class="px-6 py-4">
                                <img src="{{ route('ktp.image.show', basename($tenant->ktp)) }}" alt="Foto KTP" class="w-20 h-20 object-cover rounded-md">
                            </td>
                            <td class="px-6 py-4">{{ $tenant->dp }}</td>
                            <td class="px-4 py-3">{{ \Carbon\Carbon::parse($tenant->start_date)->format('m-Y') }}</td>
                            <td class="px-6 py-4">{{ $tenant->end_date }}</td>
                            <td class="px-6 py-4">{{ $tenant->note }}</td>
                            <td class="px-6 py-4">
                                <!-- Action Dropdown -->
                                <div class="flex items-center justify-end">
                                    <button id="action-dropdown-tenant{{ $index }}" data-dropdown-toggle="dropdown-{{ $index }}" class="inline-flex items-center p-2 text-sm font-medium text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100" type="button">
                                    ...
                                    <div id="dropdown-{{ $index }}" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                        <ul class="py-1 text-sm text-gray-700 dark:text-gray-200">
                                            <li>
                                                <button type="button" data-modal-target="updatetenant-{{ $tenant->id }}" data-modal-toggle="updatetenant-{{ $tenant->id }}" class="w-full flex items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                            ...
                                            <li>
                                                <button type="button" data-modal-target="deleteModal-{{ $tenant->id }}" data-modal-toggle="deleteModal-{{ $tenant->id }}" class="w-full flex items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 text-red-500 dark:hover:text-red-400">
